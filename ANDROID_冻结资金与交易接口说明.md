...
> **报表类（本人资料、流水、成交、风控、通知）**：见 **`ANDROID_报表查询接口对接说明.md`**（路径前缀仍为 `/api/app`，子路径 `/report/*`）。
...
- 下单成功后，系统会把 **委托价 × 委托数量 + 预估佣金 + 预估买入侧税费**（买入侧税费一般为 0）从「可用」划到「冻结」，与概要设计一致；佣金/税费费率来自 **`md_commission_rule` / `md_tax_rule`**，无规则时回退内置默认。初始化规则可执行仓库 **`sql/patch_md_trading_fee_rules_seed.sql`**。
- **可用资金减少、冻结资金增加**；其中价款部分随成交按限价×成交量释放，费用预冻部分按成交量比例释放，**最后一笔成交**一次性结清委托上剩余冻结，避免舍入残留。
- **撤单**：未成交部分对应的冻结资金**一次性退回可用**（按委托当前 `frozen_cash` 全额释放）。
...
> **限价与成交**：买入限价委托的**成交价不得高于委托价**；卖出限价委托的**成交价不得低于委托价**。模拟成交与手工回报在后端统一校验；柜台前端在提交前也会拦截，避免误报成功。
